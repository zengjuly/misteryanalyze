...
### 4.32 指数快速路径 + 缓存时效放宽（个股分析再提速 23 倍，2026-08-22）

**问题（延续 4.31）**：个股分析首次（缓存失效）仍慢——`get_market_index`
**25-46s**（在线拉指数），指数是首分析大瓶颈

**根因链**：
1. tdx-api 容器**不支持指数**（SZ399001 返回空，仅股票 SZ000001）
2. tdx_local 指数文件被**换手率检查**（指数无换手率字段→判无效）和
   **数据落后检查**（TDX 文件停在 08-20 vs 最新 08-21）拦截 → 切在线源
3. ths_official 指数支持差（重试3次×8s）→ 深成/创业板每次 20s+

**修复**：
1. `market_data_client.fetch_daily` **指数快速路径**：无条件先试 tdx_local
   （本地 .day 秒回），跳过 ths/tdx_api 重试
2. 换手率检查 + 数据落后检查对**指数豁免**（指数无换手率/本地文件即权威）
3. `data_feeder.get_market_index` 指数缓存时效放宽：**滞后 ≤3 天视为最新**
   （TDX 未同步 08-21 属正常），不重拉在线源
4. `tdx_api_client` 修复：接口要求**带交易所前缀大写代码**（SZ000001）
   ——原来传纯数字必失败（已修但指数仍不支持，股票生效）

**效果**（sz302132 首次分析全链路）：
- get_market_index：**46s → 1.3s**（指数缓存命中）
- 首次分析总耗时：**26s → 1.12s（23倍提速）**
...
### 4.37 个股页前端体验优化（docs/082208.md，2026-08-22）

**优化（用户方案）**，唯一主文件 `web/pages/1_📈_个股分析.py`：

| 项 | 做法 |
|----|------|
| 图表懒加载 | st.tabs 三图 → **st.radio 按需生成**（默认只画日线） |
| 自选股 session | list_all 进 session（TTL 5分钟 + 🔄 按钮强制刷新） |
| 分阶段 spinner | ①加载日K ②综合信号 ③加载财务 ④绘制K线 分段提示 |
| 搜索 | len(term)<2 只匹配代码前缀，不扫名称全表 |
| 最近 20 日 | 已删除（082203 时无表，导出只留信号摘要） |
| 代码无点 | 已有（code.replace('.','') + 名称解析） |

**验收（全部通过）**：
- st.tabs = 0（懒加载生效）
- 搜索 len<2 前缀匹配
- 自选 watchlist_force_refresh（TTL+按钮）
- 分阶段 spinner ①②③④ 全部存在
- 语法 OK
